...
<a href="cartview.php" class="nav-link text-white position-relative">
    ...
    <div class="container-fluid pt-5">
        <div class="d-flex flex-column text-center mb-5 pt-5">
            <h4 class="text-secondary mb-3">Final Your Order</h4>
...
$cartQuery = "SELECT DISTINCT Regdno FROM cart where cemail='$uemail' and Status='No'"; 
...
while ($cartRow = $cartResult->fetch_assoc()) {
    ...
    $petQuery = "SELECT * FROM petsale WHERE Regdno='$r' LIMIT 1"; 
    $petResult = mysqli_query($con, $petQuery);

    if ($petRow = $petResult->fetch_assoc()) { 
        $pt = $petRow['pettype'];
        $br = $petRow['Breed'];
        $c = $petRow['Color'];
        $pi = $petRow['PetImage'];
        $rate = floatval($petRow['rate']); 
        $sum += $rate; 
?>
        <tr class="text-center">
            <td><?php echo $r; ?></td>
            ...
            <td><?php echo $br; ?></td>
            <td><?php echo $pt; ?></td>
            <td><img src="img/<?php echo $pi; ?>" class="img-fluid" height="100" width="100" border="1"></td>
            <td><?php echo $rate; ?></td>
        </tr>
<?php
    }
...
<tr class="font-weight-bold text-center">
    <td colspan="4"></td>
    <td>Net Bill</td>
    <td><?php echo $sum + $gbill; ?></td>
</tr>
<tr class="font-weight-bold text-center">
    <form method="POST">
    <td colspan="6"><input type="submit" name="btnproceed" value="Proceed"></td>
</form>
</tr>
<?php 
if(isset($_POST['btnproceed']))
{
  $_SESSION['netbill']=$sum + $gbill;
  echo "<script>location.href='finalorder.php';</script>";
}
?>
